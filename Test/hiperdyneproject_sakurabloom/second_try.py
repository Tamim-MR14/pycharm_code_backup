...
for i in range(1, 15):
    for j in alp:
        mlp = MLPRegressor(max_iter = 500, solver='lbfgs', hidden_layer_sizes = (i, i), alpha = j, random_state=8)
        mlp.fit(train_data_scaled, train_target)

        acc_train.append(mlp.score(train_data_scaled, train_target))
        acc_test.append(mlp.score(test_data_scaled, test_target))


print(np.amax(acc_test))
print(np.amax(acc_train))

mlp = MLPRegressor(max_iter = 500, solver='lbfgs', hidden_layer_sizes = (2, 2), alpha = 0.01, random_state=8)
mlp.fit(train_data_scaled, train_target)

print(mlp.score(train_data_scaled, train_target))
print(mlp.score(test_data_scaled, test_target))

mlp_days = mlp.predict(test_data_scaled)
print(mlp_days.astype(int))
print(days_true)

r2_mlp = metrics.r2_score(days_true, mlp_days)
print(r2_mlp)